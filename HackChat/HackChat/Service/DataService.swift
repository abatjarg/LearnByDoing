...
class DataService {
    ...
    func uploadProfileImage(forUID uid: String, withImageUrl url: String, sendComplete: @escaping (_ status: Bool) -> ()) {
        REF_USERS.observeSingleEvent(of: .value) { (userSnapshot) in
            guard let userSnapshot = userSnapshot.children.allObjects as? [DataSnapshot] else {
                return
            }
            for user in userSnapshot {
                self.REF_USERS.child(uid).updateChildValues([
                    "provider": user.childSnapshot(forPath: "provider").value as! String,
                    "email": user.childSnapshot(forPath: "email").value as! String,
                    "profileUrl": url
                ])
            }
        }
    ...
    func getAllFeedMessages(handler: @escaping(_ messages: [Message]) -> ()) {
        // Message array to hold the messages
        var messageArray = [Message]()
        REF_FEED.observeSingleEvent(of: .value) { (feedMessageSnapshot) in
            // Grab snapshot of the message data
            guard let feedMessageSnapshot = feedMessageSnapshot.children.allObjects as? [DataSnapshot] else {
                return
            }
            // Loop through the snapshot per message
            for message in feedMessageSnapshot {
                // Grab the content of the message
                let content = message.childSnapshot(forPath: "content").value as! String
                // Grab the sender id of the message
                let senderId = message.childSnapshot(forPath: "senderId").value as! String
                // Instantiate message object using the content and senderId
                let message = Message(content: content, senderId: senderId)
                // Append it to teh messageArray
                messageArray.append(message)
            }
            // Pass back the messageArray through the handler using escaping closure (escaping to capture the values)
